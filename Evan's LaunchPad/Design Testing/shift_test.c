 ...
 *      Author: Evan Rives
 *
 *      Test program to check issue with shift operator
 *
 *      RESULT: Shift left operator has undefined behavior with signed numbers
 *      causing the audio to blow up
 *
 *      Solution: do division and multiplication directly
 *      to obtain results
 *
 */
...
int main(void)
{
    ...
    int16 factor = exp2f(4);
    while(1)
    {
        sampleIn = -110;

        sampleOut = (float32)(sampleIn / factor);
        sampleOut = (sampleOut * factor);

        sampleIn = 110;

        sampleOut = (float32)(sampleIn / factor);
        sampleOut = (sampleIn * factor);
